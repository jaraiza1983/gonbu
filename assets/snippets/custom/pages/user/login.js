        ...
        $('#m_login_signin_submit').click(function(e) {
            ...
            form.ajaxSubmit({
                ...
                data: {
					...
					ajax_login_config_item : $('[name=\"' + ajax_login_config_item + '\"]').val()
				},
				dataType: 'json',
                success: function(response, status, xhr, $form) {
                	$('[name=\"' + ajax_login_config_item + '\"]').val( response.token );
                	console.log(response);
                	
                	var error_message = "";
                	
                	if(response.status == 1){
						window.location = js_base_url + 'index.php/dashboard';
						return;
						...
						error_message = 'Failed login attempt ' + response.count + ' of ' + $('#max_allowed_attempts').val();
					}
					
                	// similate 2s delay
                	setTimeout(function() {
	                    btn.removeClass('m-loader m-loader--right m-loader--light').attr('disabled', false);
	                    showErrorMsg(form, 'danger', error_message);
                    }, 2000);
                }
            });
        });
